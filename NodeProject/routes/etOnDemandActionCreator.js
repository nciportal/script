...
class actionCreators {
	...
	onDemandInit(iObj) {
		let _actionArray = null;
	logger.info("In etOnDemandActionCreator OnDemandInit : ");
	//console.log(iObj);
	if(iObj.hasOwnProperty('baton')){
		iObj.baton.push({Location: 'etOnDemadnActioncreator_onDemandInit : node', Timestamp: new Date().getTime()});
	}else{
		let baton = [];
		iObj.baton = baton;
		iObj.baton.push({Location: 'etOnDemadnActioncreator_onDemandInit : node', Timestamp: new Date().getTime()});
	}
	if(iObj && iObj.hasOwnProperty('dataArray') && iObj.dataArray[0].hasOwnProperty('ACTION_ARRAY') && iObj.dataArray[0].ACTION_ARRAY.hasOwnProperty(0)) {
		 _actionArray = iObj.dataArray[0].ACTION_ARRAY[0];
	}else if(iObj.extraParam.fetchParams.hasOwnProperty('action')) {
		_actionArray=iObj.extraParam.fetchParams.action;
	}
		switch(_actionArray) {
		...
		case 'GET_FEATURE_AND_PART'://Added by SSD :for getting feature and part : Amruta
		...
		case 'GET_CALENDAR_STORE_DATA' : //Added by SSD : for getting store data : Himayat
		...
		case 'FETCH_ORG_CLIENT_ORDERS': //Added by SSD : organization own orders : Sudarshan
		...
		case 'FETCH_SERVICES'://Adde ]d by SSD: OnDemand call requirement to fetch all services in old service wizard. : Sudarshan	
	    ...
	    case 'GET_AVAILABLE_SERVICEMEN': //Added by SJ : Get available service men according to client appointment slot.//Sudarshan
		...
		case 'GET_PRODUCT_STATUS'://Added by SJ : Rajshree
